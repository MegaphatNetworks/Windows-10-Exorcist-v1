...
Function Global_SetSystemPolicies {
		...
		#regSet "HKLM:\SYSTEM\CurrentControlSet\Services\W32Time\Parameters\Type" "(Default)" "NT5DS" #Sets time from Domain (for domain joined computers only)
		regSet "HKLM:\SYSTEM\CurrentControlSet\Services\W32Time\Parameters\Type" "(Default)" "NTP" #Sets time from predefine list (for non-domain joined computers)
		regSet "HKLM:\SOFTWARE\Policies\Microsoft\W32time\TimeProviders\NtpClient" "Enabled" 1 #Enables time sync
		regSet "HKLM:\SYSTEM\CurrentControlSet\Services\W32Time\TimeProviders\NtpClient\SpecialPollInterval\NTPClient" "SpecialPollInterval" 604800 #set time query for 7 days (604800/3600/24)
